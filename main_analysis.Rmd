...
Furthermore, Study 3 shows that this technique can extended beyond fictional vignettes to naturally occurring text. Using tools from computational natural language processing, it is possible to extract subject-verb-object triplets from text corpora. These triplets, in turn, have the same grammatical structure, which allows for their "translation" into EPA scores. The results from Study 3 - especially the analysis around Hypothesis 1 - show that translating these events yields insightful and expected results, thus lending credibility to the external validity of the survey findings. The ability to map connotative meanings to naturally occurring text is powerful, opening up the possibility of testing a host of hypotheses about morality. The possibilities increase further when we consider that there is currently an active stream of research that attempts - quite successfully - to leverage machine learning techniques to derive EPA scores for terms that are not in the collected dictionaries. This would open the gates for exploring the connotative structures underpinning moral decision-making in a substantial amount of corpora, across cultures and throughout time. 
...
Lastly, Study 3 shows the horizon of possibilities that natural languge processing offers to the study of morality but, as with many nascent compuational methods, the techniques are prone to error and missclassification. For instance, the parts-of-speech tools I implement to extract the triplets cannot recognize sentences with complex structures, where the object might be implied or carried over from other parts of the text. Thus, I am only capturing a fraction of the events present in the text; those that occur in sentences with a similar grammatical structure. Moreover, sentiment analysis is correctly identified as a relatively blunt tool, incapable of discerning important context cues, like sarcasm. Despite these shortcomings, the results remain insightful, showing that by looking at connotative meanings, researchers can use naturally occurring text to probe at interesting and difficult problems. 
...
Furthermore, these studies shed light on the ongoing debates about the existence of "harmless" wrongs. They show that even the so-called purity transgressions are understood through the lens of harm and that events that are more distant from the prototypical violation are less likely to be classified as harmful. However, these analyses also suggest that these transgressions are indeed distinctive: they are quite distant from the template and yet they are considered very harmful. The question, then, should not be how harmless wrongs become immoral but rather how events that are far away from prototypical moral transgressions are "harmified". Here, I offer evidence that suggests that more moral vocabulary occurs around these distinctive event. I contend that this lends credence to the idea that more moral cultivation is needed to perceive harmfulness in these acts; it is necessary to construct moral and cultural frameworks around these events to bring them to the realm of harm. 

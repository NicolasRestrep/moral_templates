...
- \usepackage{setspace}
- \doublespacing
bibliography: templates_references.bib
abstract: "Individuals and groups see immorality in different places. Events that are everyday to some might cause outrage to others. While there has been progress in understanding moral decision-making, more research is needed to elucidate the cognitive processes that underpin the attribution of immorality. In this paper, I use sociological approaches to cultural meaning in order to test one of the leading theories of moral cognition: the idea that individuals attribute immorality through template matching. Analyzing the connotative structures of moral trangressions allows me to define what prototypicality means in the context of moral violations. Then, using reaction-time data, I examine whether deviations from such prototypicality inform how long it takes individuals to categorize scenarios as either immoral or harmful. Lastly, I test the survey results on a relatively large corpus of naturally occurring text in order to assess their external validity. These studies allow me to provide concrete empirical evidence that supports the notion that the attribution of immorality occurs through template matching; a theory that, while increasingly popular, has remained largely untested"
---
...
Figure 1 shows the relationship between the average immorality of each scenario and its average harmfulness and average unexpectedness. The results further reaffirm the close relationship between immorality and harm. Moreover, the positive relationship between immorality and unexpectedness echoes the idea that severe transgressions tend to be perceived as more unexpected because they often entail the breaching of social norms. For the purposes of this study, the most important point is that the translated scenarios are interpreted consistently. There are encouraging patterns that are consistent with findings in the literature: the “harm” transgressions tend to be rated as the most severe, while the “purity” violations rank among the most unexpected. This suggests that there was not a significant loss of information when the vignettes were translated into the new format.
...
A concern with these results is that most of the scenarios that are very distant from the prototype are the so-called purity scenarios. Study 1 shows that these scenarios are consistently ranked among the most harmful. Thus, it is possible that these transgressions are driving the downward turn of the quadratic effect. Nonetheless, I ran the same models taking these scenarios out of the sample and the quadratic relationship still held; these results are available in the supplemental materials. More interestingly, however, it is possible to fit a logistic regression predicting whether an event was classified as harmful using distance from the prototype as the main independent variable. Figure 6 shows the coefficients of this multi-level, cross-classified model. Increases in distance from the prototype lead to considerably lower odds of an event being classified as harmful. Again, it is important to re-emphasize that the so-called purity scenarios are both quite distant from the prototype and yet are very harmful. Thus, these scenarios are "out of place": their connotative structures do not seem to correspond to the harmfulness that participants attribute to them. 

This "mismatch" between connotative structures and perceived harm speaks directly to an important debate within the study of morality. A great deal of research within this line of work has revolved around the so-called harmless wrongs [@tepeHarmfulnessImpurityMoral2019]. As noted above, scholars have recently argued that these acts are still perceived through the lens of harm. Indeed, the results of Study 1 echo this idea: the so-called purity transgressions rank among the most harmful. Yet, the mismatch above suggests that these events might be indeed distinctive; harmless wrongs are "weird" - in Gray and Schein's words - but not only because they are unusual but because their connotative meanings do not fit with their perceived harmfulness. The challenge, then, is to explain this incongruence. One explanation that can be put forward is that there are different channels for “harmification” [@scheinMoralizationHarmificationDyadic2016]. While perceiving the harmfulness of events that are very close to the prototype might be intuitive, doing the same for practices that are far away might require a longer process of social cultivation. Therefore, it might be necessary for groups to build additional moral scaffolding – such as taboos and mechanisms of public shame – in order to bring events that are very far away from the template into the realm of harm. I will build upon this explanation, which remains speculative, to outline the hypotheses that will serve to test the external validity of the results obtained so far. 
...
In order to translate the findings into hypotheses that can be tested using natural language processing tools, I focus on sentiment analysis. While a host of different research agendas tend to fall under this rubric, they are united in the effort to infer the emotional valence of written text. The results from Study 1 & 2 suggest that events that are close to the template should be surrounded by more negative emotional content than those that are distant from it. The first hypothesis, then, is: 

> Hyp. 1: The negative sentiment of the text surrounding an event should be negatively related to how distant said event is from the moral template. 

This relationship is not as obvious as it might seem at first. Recall that the semantic attributes that go into calculating distance from the template are the behavior's evaluation, the behavior's potency, and the object's potency. There's only one measure that would map to goodness/badness directly. Therefore, identifying a relationship between distance from the template and negative sentiment would not be trivial.

A perhaps more interesting hypothesis relates to the speculative explanation about differential "harmification" outlined above. This explanation suggests that additional work is needed to bring events that are very distant from the prototype into the realm of harm. In text, we can conceptualize this additional work as moral language: language that is used to set normative standards, and to denounce or even justify deviations from such standards. I elaborate on this operationalization below, but what is important here is to outline the statistical relationship that should be expected given the results above. In general, we should expect such moral vocabulary to appear at higher rates around events that are close to the template. However, there should be an interaction effect between distance from the template and negative sentiment such that, for events that are surrounded by really negative sentiment, the relationship between distance and the rate of moral vocabulary should be positive. Events like the so-called purity scenarios, which are considered both really negative and really distant from the template, should have more moral scaffolding around them. The second hypothesis, thus, is: 

> Hyp. 2: In considering the count of moral vocabulary around an event, there should be a positive interaction effect between negative sentiment and distance from the moral template. 

### Data and Methods 

For this analysis, I use a dataset consisting of all articles published by the news site Vox before the 21st of March 2017. This dataset was collected by Elena Zhelenko and made available to the public through the social platform data.world. In total, the corpus contains 23022 articles published between 2014 and 2017. These data prove particularly valuable for the problem at hand because Vox covers a wide array of topics, from economics to public policy and entertainment. This ensures that I am able to capture a relatively wide group of events, significantly broadening the scope of the studies above. 

In order to test the hypotheses, it is necessary to extract events from these articles; events that can be comparable in structure to the ones used in the previous studies. The automated identification of subject-verb-object triplets in naturally occurring text is an area of active research and the last few years have seen a proliferation of user-friendly and powerful tools for this task. In this project, I use the Python libraries spaCy and textacy, which offer automated parts-of-speech recognition tools that allow to identify and extract these triplets. As a concrete example, take the following sentence from one of the articles:
...
After extracting all recognizable triplets from each article, there was a total of 522423 such events. I then filtered these triplets to identify the events for which there were existent EPA values. Here, it is important to recall that the measure of distance used in study 2 takes into account the EPA scores of the behavior and the object. Therefore, these were the two components I took into account when mapping EPA scores to the identified triplets. The resulting data-set contains 10274 triplets, spanning 6477 articles, for which the necessary EPA scores are available. This means that I can map the affective meanings onto these triplets, which in turn allows me to calculate their distance to the moral template. 

To operationalize the notion of "moral scaffolding", I compiled a dictionary of moral vocabulary. As a starting point, I used the "general morality" section of the Moral Foundations Theory dictionary, focusing on the negative terms. I then proceeded to find appropriate synonyms in order to have a more comprehensive list. The final dictionary consists of 169 terms and it is made up of words that are used to describe norms and duties (e.g. "integrity", "ought"), others that are used to denounce (e.g. "atrocious"), and words that are implemented to justify (e.g "alibi", "condone"). This operationalization is by no means perfect, but it is an effective tool to detect the presence of moral scaffolding - explanations, assertions and/or denunciations about what ought and should not be done - in text. The results above suggest that distance from the moral template should be informative for predicting the amount of words from this dictionary that occur around a given event. 

Hypothesis one posits a negative relationship between how distant an event is from the moral template and the negative sentiment of the text around said event. To measure the sentiment around a given event, I first extract an excerpt of text consisting of the fifty words prior to the first word of the triplet and the fifty words following its last. Then, I use the NLTK library sentiment analyzer - VADER - to compute the sentiment for the desired excerpt. Having the sentiment scores for all triplets, I conduct a linear regression, where the outcome variable is negative sentiment and the main explanatory variable is the event's distance from the prototype. Evidencing a negative relationship would lend credibility to the results of the previous studies. 

To test the second hypothesis, I use the same excerpts of text around each event, and count the number of words that also appear in the compiled dictionary of moral vocabulary described above. Before checking for matches, the words in both the excerpts and the dictionary are lemmatized and stemmed. This helps identify overlaps, even if modified versions or alternative spellings of the words are used. In this analysis, the outcome variable is the amount of words in each excerpt that coincide with the terms in the dictionary. Given that this is a count variable, a Poisson regression here would be the most appropriate. Thus, I regress the count of matches on the event's distance from the template, the negative sentiment surrounding it, and an interaction between these two terms. 

### Results 

Figure 7 depicts the predicted line of best fit of a simple linear regression with the negative sentiment around the event as the outcome variable and its distance from the moral template as the explanatory variable. The relationship lends plausibility to Hypothesis 1; a unit increase in distance from the prototype results in a 0.3 standard deviations decrease in negative sentiment. As events move away from the moral template, there is less negative sentiment around them.

The analysis for Hypothesis 2 also shows supportive evidence. Figure 8 depicts a coefficient plot for the Poisson regression, where the dependent variable is the count of moral terms around an event. It is immediately noticeable that all coefficients are in the direction we would expect given the results above. As the negative sentiment around events increases, so does the count of moral vocabulary around them. Most of the terms in the compiled dictionary have negative connotations, so it is unsurprising to see this relationship. Nonetheless, the other two coefficients are perhaps more insightful. As distance from the template increases, the rate at which we find moral words decreases, as Gray and Schein's (2018) theory would suggest. The crucial coefficient here, however, is the interaction term between distance from the template and negative sentiment: the positive coefficient means that, in triplets that are very negative, we should expect the relationship between distance from the template and the count of moral terms to actually be positive. This result lends credence to the notion that events that are very negative and yet very far away from the moral template - like the so-called purity violations - should have more moral scaffolding around them. It is important to take into account, however, that by definition there are not too many cases in such extreme values of negative sentiment and that, therefore, we need to be cautious about the conclusions we can draw from these results. While these insights are by no means definitive, they are suggestive: they add plausibility to the survey results and elucidate a possible explanation for the mismatch between the perceived harmfulness and connotative structures of the so-called harmless wrongs. 
...
plot_model(first_model, title = "Negative Sentiment and Distance", type = 'pred', terms = c('dist_stand'))
  
poiss_mod <- glm(moral_count ~ dist_stand*neg_stand, data = df, family = "poisson") 
plot_model(poiss_mod, sort.est = T, title = "Poisson Regression - Count of Moral Words", show.values = T)
...
Furthermore, Study 3 shows that this technique can extended beyond fictional vignettes to naturally occurring text. Using tools from computational natural language processing, it is possible to extract subject-verb-object triplets from text corpora. These triplets, in turn, have the same grammatical structure, which allows for their "translation" into EPA scores. The results from Study 3 - especially the analysis related to Hypothesis 1 - show that translating these events yields insightful and expected results, thus lending credibility to the external validity of the survey findings. The ability to map connotative meanings to naturally occurring text opens up the possibility of testing a host of hypotheses about morality. The possibilities increase further when we consider that there is currently an active stream of research that attempts - quite successfully - to leverage machine learning techniques to derive EPA scores for terms that are not in the collected dictionaries. This would would allow us to explore the connotative structures underpinning moral decision-making in a considerable amount of corpora, across cultures and throughout time. 

By establishing these connotative regularities, I am able to outline a formal definition of what the template of an immoral act looks like and to calculate deviations from said template. Finding important semantic dimensions for the attribution of immorality helps me place the scenarios on a plane defined by those dimensions. By “maximizing” the values on these dimensions, I can represent the prototypical moral wrong as a point in this space and, therefore, I can calculate how distant or close the scenarios are from this template. This allows me to directly test the proposition that moral cognition occurs through template matching. The results from Study 2 show that distance from the template is indeed informative of how long it takes individuals to classify events as harmful. Moreover, the findings from the last study suggest that distance from the template has external validity beyond the highly curated set of moral vignettes, yielding information about the sentiment that is found around events in naturally occurring text. Interestingly, the results from Study 2 also suggest how this relationship should be understood, a point that Gray and Schein [-@scheinTheoryDyadicMorality2018] do not elaborate on. The relationship is quadratic: as distance increases reaction time increases, but only up to a point. Individuals are quick to classify events that are very close or far apart from the template. Events that lie at a middling distance are more ambiguous and take longer to be perceived as harmful. These findings, then, provide empirical evidence for the notion that template matching mediates moral cognition, but also give a clearer idea of how this process occurs. 

These studies also shed light on one of the most important debates within the study of moral cognition. Many of the recent theoretical developments in the study of morality have been animated by the question of whether there are such things as harmless wrongs [@tepeHarmfulnessImpurityMoral2019]. These scenarios tend to be associated with the “purity” foundation: they often involve sexual or dietary taboos. As noted above, MFT arose as an attempt to incorporate these cases, while Gray and Schein’s work could be seen as a response to these categorization efforts. The results from Study 1 side with the latter, noting that some of the so-called “purity” transgressions are perceived as very harmful. Yet, the findings from Study 2 compel us to rethink the distinctiveness of these scenarios. They are quite far away from the prototype and, appropriately, they tend to be classified fairly quickly. However, the classification goes contrary to what the model would predict: they tend to be considered harmful. These violations, then, break the patterns that the models uncover in the data. Both sides of the debate, then,  seem to have pointed out important aspects of this type of transgressions. Gray and Schein [-@scheinTheoryDyadicMorality2018] are right in insisting that that purity scenarios are understood through the lens of harm. However, these transgressions *are* distinctive insofar as they considered very harmful and yet are quite far away from the prototypical moral wrong. My findings suggest that we should move away from the debate about “harmless” wrongs, but they compel us to consider, then, how practices that are distant from the moral template come to be regarded as harmful. One alternative, for instance, is that there are different processes of “harmification” [@scheinMoralizationHarmificationDyadic2016]. While harm might be readily apparent in events that are close to the template, it might be harder to discern in situations that are really distant from it. Thus, more moral elaboration – in the form of norms, denunciations, and justifications – might be needed in order to teach members of a group how to spot harmfulness in such events. In Study 3, I sought to test this hypothesis and found some evidence for its plausibility. In the corpus of Vox articles, events that are surrounded by negative sentiment have more moral vocabulary around them. As events move away from the template, the rate of moral vocabulary around them decreases. However, there is a positive interaction term between distance and negative sentiment. This means that at really high levels of negative sentiment, as events move away from the template they should expect to be surrounded by more moral vocabulary. So if events are really far away from the template and really negative – like the so-called purity violations –, there should be more moral scaffolding around them. This is by no means a definitive finding but only an indication of the plausibility of this explanation. More importantly, however, this finding shows how we can harness natural language processing tools and semantic analysis to shed light on central debates in the study of moral cognition. 

## Limitations and Future Directions

An important limitation of studies 1 and 2 concerns the number and type of fictional vignettes used. The key independent variables in these studies are related to the semantic structures of the events. Therefore, given that I only use 25 vignettes, I rely on limited variation to produce my estimates. This might be why some of the coefficients have large confidence intervals. Furthermore, the events are all practices that are relatively immoral. This might sound obvious but effectively what I am doing here is a form of selecting on the dependent variable. I am making an argument about a general process of template matching using only a particular kind of practice. Study 3 tries to ammeliorate this by testing the findings on a much wider - and less curated - set of events, showing the external validity of the results. However, I arrived at the hypotheses tested using a limited sample of events, which fail to comprehensively encompass the semantic space. Future work should focus on expanding this semantic analysis, systematically exploring events with widely varying connotative structures. 

Another potential shortcoming of this analysis is related to the sensitivity of one of the instruments. Reaction time studies are fickle and here I am adding complexity to this task. I am asking participants to react to full sentences and to do so remotely, through a crowdsourcing website. Though the data show consistent and encouraging patters, these limitations should be taken into account when drawing conclusions from these studies. In future work, it would be useful to find ways of measuring moral cognition more precisely. It could be possible, for instance, to use eye-tracking measures or to adapt novel techniques – such as Affect Misattribution Procedures – to the analysis of moral cognition. 

Lastly, Study 3 shows the space of possibilities that natural language processing offers to the study of morality but, as with many nascent computational methods, the techniques are prone to error and misclassification. For instance, the parts-of-speech tools I implement to extract the triplets cannot recognize sentences with complex structures, where, for instance, the object might be implied or carried over from other parts of the text. Thus, I am only capturing a fraction of the events present in the text; those that occur in sentences with a similar and relatively straightforward grammatical structure. Moreover, sentiment analysis is correctly identified as a relatively blunt tool, incapable of discerning important context cues like sarcasm or irony. Despite these shortcomings, the results remain insightful, showing that by looking at connotative meanings, researchers can use naturally occurring text to probe at interesting and difficult questions in the study of morality. 

## Conclusion 

These studies represent a concrete attempt to test one of the most plausible contemporary theories of how moral cognition operates. Current evidence supports a more unified vision of moral judgment, where perceptions of harm are the main drivers of this process. Gray and Schein have outlined a model of moral cognition that is consistent with this vision; one where immorality is ascribed through template matching. The evidence to support this position, however, remains limited. Here, I device a technique for directly examining their conceptualization. I leverage existent repositories of cultural meanings to look at the underlying semantic structures of moral transgressions. This, in turn, allows me to accomplish two things: to produce a rigorous formal definition of a prototypical moral wrong, and to examine whether deviations from that template affect how individuals perceive transgressions. The results suggest that distance from the prototypical moral transgression is indeed informative of the time it takes individuals to classify scenarios as harmful. Furthermore, distance from the template is also predictive of the sentiment around events in naturally occurring text. This evidence adds plausibility to the idea that the attribution of immorality happens through template matching. 

Furthermore, these studies shed light on the ongoing debates about the existence of "harmless" wrongs. They show that even the so-called purity transgressions are understood through the lens of harm and that events that are more distant from the prototypical violation are less likely to be classified as harmful. However, these analyses also suggest that these transgressions are indeed distinctive: they are quite distant from the template and yet they are considered very harmful. The question, then, should not be how harmless wrongs become immoral but rather how events that are far away from prototypical moral transgressions are "harmified". Here, I offer evidence that suggests that more moral vocabulary occurs around these distinctive event. I contend that this lends credence to the idea that more moral cultivation is needed to perceive harmfulness in these acts; it is necessary to construct moral and cultural frameworks around these events to bring them into the realm of harm. 

This paper should be regarded as an initial foray into the empirical analysis of the role that template matching plays in the attribution of immorality. As noted above, there are important limitations that compel me to be cautious about the claims that can be advanced with my evidence. The variation between the semantic structures of the events is limited and reaction time data can be unreliable. Moreover, the natural language processing techniques can be prone to misclassification and fail to deal with complex grammatical structures. Nonetheless, I have taken the necessary steps to make this analysis as rigorous as possible. Thus, despite the limitations, this study represents a novel attempt to provide empirical grounding to a theory that, though popular, remains largely untested. By analyzing the semantic structures of moral transgressions, I present evidence that suggests that the attribution of immorality occurs through template matching. Future work should continue to analyze connotative regularities among moral violations in order to further our understanding of how moral cognition operates and how it varies systematically across groups. 
